      ...
      <div className="bg-white text-slate-900 rounded-2xl max-w-3xl w-full p-6 sm:p-8 shadow-2xl relative border border-slate-200 print:m-0 print:p-0 print:border-none print:shadow-none">
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-200 print:hidden">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handlePrint} className="px-3.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold flex items-center gap-1.5 transition-colors shadow-sm">
        ...
        <div className="space-y-5">
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden">
              ...
              <div className="p-3 space-y-2">
                ...
                {record.incentivesList && record.incentivesList.filter((i) => i.isQualified).map((inc, i) => (
                  <div key={i} className="flex justify-between pl-3 text-[11px] text-slate-500"><span>• {inc.name}:</span><span className="font-mono">+₱{inc.amount.toFixed(2)}</span></div>
                ))}
              </div>
            </div>
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden">
              ...
              <div className="p-3 space-y-2">
                ...
                {record.deductionsList && record.deductionsList.map((ded, i) => (
                  <div key={i} className="flex justify-between py-1 border-b border-slate-100"><span className="text-slate-600">{ded.name}:</span><span className="font-mono font-semibold text-rose-700">₱{ded.amount.toFixed(2)}</span></div>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-900 text-white p-4 rounded-xl flex items-center justify-between shadow-md">
            <div><span className="text-xs font-semibold text-blue-200 block uppercase tracking-wider">Official Net Pay (Take Home)</span><p className="text-xs text-blue-300 mt-0.5">Disbursed via direct payroll crediting</p></div>
            <div className="text-right"><span className="text-2xl sm:text-3xl font-black font-mono tracking-tight text-white">₱{record.netSalary.toFixed(2)}</span></div>
          </div>
